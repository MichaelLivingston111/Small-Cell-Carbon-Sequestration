...
# Stack the data:
stacked_data <- select(boxplot_data, c('Type', 'Region', 'Small_uptake', 'Large_uptake'))
stacked_dataframe <- na.omit(cbind(stacked_data[c(1, 2)], stack(stacked_data[c(3, 4)])))
...
C_stacked_df$Region <- factor(C_stacked_df$Region)
C_stacked_df$ind3 <- as.numeric(C_stacked_df$ind)

dodge <- position_dodge(width = 0.9)

C_stacked_df$Region3 <- as.numeric(C_stacked_df$Region) + 
...
N_stacked_df$Region <- factor(N_stacked_df$Region)
N_stacked_df$ind3 <- as.numeric(N_stacked_df$ind)

dodge <- position_dodge(width = 0.9)

N_stacked_df$Region3 <- as.numeric(N_stacked_df$Region) + 
  3 * (as.numeric((N_stacked_df$ind)) - 0.2)
...
NP_stacked_df$Region <- factor(NP_stacked_df$Region)
NP_stacked_df$ind3 <- as.numeric(NP_stacked_df$ind)

dodge <- position_dodge(width = 0.9)

NP_stacked_df$Region3 <- as.numeric(NP_stacked_df$Region) + 
...
Si_stacked_df$Region <- factor(Si_stacked_df$Region)
...
ridges_data <- select(data, c('Region', 'Large_POC_PON', 'Small_POC_PON', 'POC_PON'))
...
ridges_plot <- stacked_ridges %>%
  ...
  ggplot(aes(x = RR, y = Region, group = interaction(ind, Region), fill = ind)) +
...
df_SCC <- data %>% select(Percent_Nano_Silica, Percent_Nano_Carbon, Percent_Nano_Nitrate, Percent_Nano_NP, 
                              Nitrate, Silicate, Temp, MLD, Sigma, PAR, DO, DFS, Region, Region2, Season,
...
PCA_output <- function(df, df2) {
  ...
  plot <- autoplot(pca, data = df2, colour = 'Region', loadings = TRUE, 
         loadings.colour = 'blue',loadings.label = TRUE, 
         loadings.label.size = 3, loadings.label.colour = 'black', 
         frame = T, frame.colour = "Region", frame.type = 't')
